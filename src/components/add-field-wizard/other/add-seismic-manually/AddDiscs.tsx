...
import { SeismicDisc } from "@/types/models/seismicDisc";
...
import {
  ...
  profileTypes,
  referenceSystems,
} from "@/utils/constants/seismic";
import { toast } from "react-toastify";

const initialState: SeismicDisc = {
...
type AddDiscsProps = {
  seismic: Seismic;
  onCompleted: (seismic: Seismic, newSeismic: boolean) => void;
};

export const AddDiscs = ({ seismic, onCompleted }: AddDiscsProps) => {
  const [discData, setDiscData] = useState<SeismicDisc>(initialState);
  const [discs, setDiscs] = useState<SeismicDisc[]>([]);
  ...
  const handleProceedAndAddNewSeismic = () => {
    handleSave();
    const newSeismic = { ...seismic, discs };
    onCompleted(newSeismic, true);
    toast.success("Redirected to add new Seismic!");
  };

  const handleComplete = () => {
    handleSave();
    const newSeismic = { ...seismic, discs };
    onCompleted(newSeismic, false);
  ...
  return (
    <>
      ...
      <div className="flex flex-row gap-6 pt-4">
        ...
        <div
          className="bg-black text-white justify-between w-1/3 py-2 rounded text-center cursor-pointer"
          onClick={handleProceedAndAddNewSeismic}
        >
          Proceed & Add New Seismic
        </div>
        <div
          className="bg-black text-white justify-between w-1/3 py-2 rounded text-center cursor-pointer"
