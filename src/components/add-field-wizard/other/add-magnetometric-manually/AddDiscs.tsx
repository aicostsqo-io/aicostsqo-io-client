...
import { MagnetometricDisc } from "@/types/models/magnetometricDisc";
...
import {
  ...
  profileTypes,
  referenceSystems,
} from "@/utils/constants/magnetometric";
import { toast } from "react-toastify";

const initialState: MagnetometricDisc = {
...
type AddDiscsProps = {
  ...
  onCompleted: (
    magnetometric: Magnetometric,
    newMagnetometric: boolean
  ) => void;
};

export const AddDiscs = ({ magnetometric, onCompleted }: AddDiscsProps) => {
  const [discData, setDiscData] = useState<MagnetometricDisc>(initialState);
  const [discs, setDiscs] = useState<MagnetometricDisc[]>([]);
  ...
  const handleProceedAndAddNewMagnetometric = () => {
    handleSave();
    const newMagnetometric = { ...magnetometric, discs };
    onCompleted(newMagnetometric, true);
    toast.success("Redirected to add new magnetometric!");
  };

  const handleComplete = () => {
    handleSave();
    const newMagnetometric = { ...magnetometric, discs };
    onCompleted(newMagnetometric, false);
  ...
  return (
    <>
      ...
      <div className="flex flex-row gap-6 pt-4">
        ...
        <div
          className="bg-black text-white justify-between w-1/3 py-2 rounded text-center cursor-pointer"
          onClick={handleProceedAndAddNewMagnetometric}
        >
          Proceed & Add New Magnetometric
        </div>
        <div
          className="bg-black text-white justify-between w-1/3 py-2 rounded text-center cursor-pointer"
